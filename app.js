...
function updateUI(){
    ...
    filteredList.forEach(function(item,index){
        ...
        itemElement.classList = "p-4 border border-gray-200 rounded-lg flex justify-between items-center bg-gradient-to-r from-blue-50 to-indigo-50";
        itemElement.innerHTML=`<div> <p class='font-semibold'> ${item.description}</p> 
        <p class='text-sm text-gray-600'> ${item.category}- target:  ${item.targetDate}</p></div><div> 
        <button onclick='editItem(${index})' class='px-2 py-1 bg-yellow-500 text-white rounded'>Edit</button> 
        <button onclick='toggleCompletion(${index})' class='ml-2 px-2 py-1 rounded ${(item.completed ? "bg-green-500 text-white" : "bg-red-500 text-white")}'>${(item.completed ? "Achieved" : "Pending")} </button>
        <button onclick='deleteItem(${index})' class='ml-2 px-2 py-1 bg-red-500 text-white rounded'>Delete</button></div>`;
